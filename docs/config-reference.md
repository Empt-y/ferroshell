...
| `org.ferroshell.battery` | `show-percentage`, `hide-without-battery`. Battery level and time left, the power mode (needs the Balanced plan), a brightness slider per screen (laptop panels, and external monitors with DDC/CI), energy saver status. Scroll over it to change brightness. In replacement mode, brightness changes (e.g. Fn keys) show an OSD |
| `org.ferroshell.notifications` | `show-count`; shell settings `banners` (`"auto"`: only while Ferroshell replaces Explorer, `"always"`, `"never"`) and `banner-seconds`. Other apps' notifications grouped by app, with dismiss/clear and Do Not Disturb. Needs package identity: see `packaging/identity/README.md`. Not in the default config |
